<?php 
  ...
  function cambiarFormatoFecha($fecha){ 
	...
	$query =	"select idtiporef, docref,
						...
						and ltrim(substring(docref from position('-' in docref)+1 for 2)) = '".$tipo_comp."'
 ...
 // 	echo $moneda."<br>";
	...
	$cad = $cad.'tipo_de_comprobante|'.$tipo_comp_num.'|'.$salto;
	$cad = $cad.'serie|'.$tipo_comp.$tipo_comp.$tipo_comp.'1'.'|'.$salto;
	...
	$query = "SELECT tbdetpedpres.*, tbproducto.* 
						FROM tbdetpedpres, tbdetprodpres, tbproducto
						WHERE tbdetpedpres.idpresprod = tbdetprodpres.idpresprod
						AND tbdetprodpres.idprod = tbproducto.idprod
...
//item|NIU|432|SUMINISTRO TINTA EPSON 664 NEGRO T664120|2|23.73|28||23.73|1|4.27|28|false|||
 ...
 //                        "anticipo_documento_numero" => ""
	...
	// header("Location: enviar_nube.php?idped=".$idpedido);
